...
TMC2208Stepper::TMC2208Stepper(Stream * SerialPort, float RS, uint8_t addr, uint16_t mul_pin1, uint16_t mul_pin2) :
	...
	// Protected
	// addr needed for TMC2209
	TMC2208Stepper::TMC2208Stepper(int16_t SW_RX_pin, int16_t SW_TX_pin, float RS, uint8_t addr) :
		TMCStepper(RS),
	...
	void TMC2208Stepper::beginSerial(uint32_t baudrate) {
		if (SWSerial != nullptr)
		{
			...
			SWSerial->enableIntTx(true);
			SWSerial->enableIntTx(false);
			#if defined(ESP32)
				if(RXTX_pin > 0)
				{
					SWSerial->enableTx(true);
				}
			#else
				SWSerial->end();
			#endif
