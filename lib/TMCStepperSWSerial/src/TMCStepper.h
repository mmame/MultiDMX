...
class TMC2208Stepper : public TMCStepper {
	public:
		...
		TMC2208Stepper(Stream * SerialPort, float RS) :
			...
			{}
		#if SW_CAPABLE_PLATFORM
			TMC2208Stepper(int16_t SW_RX_pin, int16_t SW_TX_pin, float RS) :
			...
			TMC2208Stepper(int16_t SW_RX_pin, int16_t SW_TX_pin, float RS, bool) :
				TMC2208Stepper(SW_RX_pin, SW_TX_pin, RS, TMC2208_SLAVE_ADDR)
				{};
		#else
	...
	protected:
		...
		#if SW_CAPABLE_PLATFORM
			TMC2208Stepper(int16_t SW_RX_pin, int16_t SW_TX_pin, float RS, uint8_t addr);
		#endif

		Stream * HWSerial = nullptr;
		#if SW_CAPABLE_PLATFORM
			SoftwareSerial * SWSerial = nullptr;
			const int16_t RXTX_pin = 0; // Half duplex
		#endif
		...
		static constexpr uint8_t replyDelay = 20;
...
class TMC2209Stepper : public TMC2208Stepper {
	public:
		...
		#if SW_CAPABLE_PLATFORM
			TMC2209Stepper(int16_t SW_RX_pin, int16_t SW_TX_pin, float RS, uint8_t addr) :
